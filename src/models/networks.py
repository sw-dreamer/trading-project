"""
SAC 알고리즘의 Actor와 Critic 네트워크 구현 모듈 (LSTM 추가 버전)
"""
import torch
...
class LSTMActorNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(lstm_hidden_dim + hidden_dim // 4, hidden_dim)
        self.fc2 = nn.Linear(hidden_dim, hidden_dim)
...
class LSTMCriticNetwork(nn.Module):
    ...
    def forward(self, state: Dict[str, torch.Tensor], action: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        q1_lstm_features = q1_lstm_out[:, -1, :]  # 마지막 타임스텝의 출력
        
        # 포트폴리오 상태 처리
        q1_portfolio_features = F.relu(self.q1_portfolio_fc(portfolio_state))
        
        # 행동 처리
        q1_action_features = F.relu(self.q1_action_fc(action))
        
        # 특성 결합
        q1_combined = torch.cat([q1_lstm_features, q1_portfolio_features, q1_action_features], dim=1)
        ...
        q2_lstm_features = q2_lstm_out[:, -1, :]  # 마지막 타임스텝의 출력
        
        # 포트폴리오 상태 처리
        q2_portfolio_features = F.relu(self.q2_portfolio_fc(portfolio_state))
        
        # 행동 처리
        q2_action_features = F.relu(self.q2_action_fc(action))
        
        # 특성 결합
        q2_combined = torch.cat([q2_lstm_features, q2_portfolio_features, q2_action_features], dim=1)
