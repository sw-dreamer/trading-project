        ...
        try:
            ...
            if position and position.get('quantity', 0) != 0:
                try:
                    ...
                    if self.logger:
                        self.logger.warning(f"{symbol} 포지션 정보 변환 실패: {e}")
            
            # 포지션이 없거나 수량이 0인 경우 캐시에서 제거
            if symbol in self.positions:
                del self.positions[symbol]
            
            if self.logger:
                self.logger.info(f"{symbol} 포지션 없음")
            
            return default_position
            
        except Exception as e:
            if self.logger:
                self.logger.debug(f"{symbol} 포지션 정보 업데이트 실패 (포지션 없음): {str(e)}")
    ...
    def get_position(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if symbol in self.positions:
                ...
                if self.logger:
                    self.logger.info(f"📊 {symbol} 포지션 정보 (캐시):")
                    self.logger.info(f"  💰 수량: {position.get('quantity', 0):.6f}")
                    self.logger.info(f"  💵 진입가: ${position.get('entry_price', 0):.2f}")
                    self.logger.info(f"  📈 미실현 손익: ${position.get('unrealized_pnl', 0):.2f}")
                    self.logger.info(f"  💎 시장가치: ${position.get('market_value', 0):.2f}")
                return position
            ...
            if self.logger:
                self.logger.info(f"📊 {symbol} 포지션 정보 (API):")
                self.logger.info(f"  💰 수량: {position.get('quantity', 0):.6f}")
                self.logger.info(f"  💵 진입가: ${position.get('entry_price', 0):.2f}")
                self.logger.info(f"  📈 미실현 손익: ${position.get('unrealized_pnl', 0):.2f}")
                self.logger.info(f"  💎 시장가치: ${position.get('market_value', 0):.2f}")
            ...
            if self.logger:
                self.logger.debug(f"{symbol} 포지션 조회 실패 (포지션 없음): {str(e)}")
            return {
                "symbol": symbol,
                "quantity": 0,
                "entry_price": 0,
