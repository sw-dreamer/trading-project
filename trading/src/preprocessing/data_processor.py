...
class DataProcessor:
    ...
    def process_all_symbols(self, raw_data_dict: Dict[str, pd.DataFrame], use_windows: bool = False) -> Dict[str, Dict[str, pd.DataFrame]]:
        """
        모든 심볼 데이터 처리 (Data Leakage 방지 - Train Only 정규화)
        """
        ...
        LOGGER.info(f"🔄 {len(raw_data_dict)}개 심볼 Data Leakage 방지 처리 시작...")
        ...
        for i, (symbol, raw_data) in enumerate(raw_data_dict.items(), 1):
            ...
            featured = self.extract_features(preprocessed)
            
            # ✅ 2. 먼저 시간 순서로 데이터 분할 (Data Leakage 방지)
            ...
            LOGGER.info(f"   📊 분할 완료: train {len(train_data)}, valid {len(valid_data)}, test {len(test_data)}")

            # ✅ 3. Train 데이터로만 피처 선택 (미래 정보 완전 차단)
            if first_symbol:
                initial_features = len(train_data.columns)
                LOGGER.info(f"   📋 Train 데이터로만 피처 선택: {initial_features}개 피처에서...")
                train_data = self.feature_selector.fit_transform(train_data, target_col='close')
                ...
                # Valid/Test에 동일한 피처 적용
                valid_data = self.feature_selector.transform(valid_data, target_col='close')
                test_data = self.feature_selector.transform(test_data, target_col='close')
            else:
                LOGGER.info(f"   🔄 동일한 피처 기준 적용...")
                train_data = self.feature_selector.transform(train_data, target_col='close')
                valid_data = self.feature_selector.transform(valid_data, target_col='close')
                test_data = self.feature_selector.transform(test_data, target_col='close')

            # ✅ 4. Train으로만 정규화 통계 계산, Valid/Test는 Transform만
            LOGGER.info(f"   🎯 Train-only 정규화 시작...")
            ...
            LOGGER.info(f"   ✅ Data Leakage 방지 정규화 완료")
            ...
            LOGGER.info(f"   ✅ {symbol} 완료: train {len(train_data_norm)}, valid {len(valid_data_norm)}, test {len(test_data_norm)}")

        # 최종 피처 요약 출력
        if self.feature_selector and hasattr(self.feature_selector, 'selected_features'):
            LOGGER.info(f"\n🎯 Data Leakage 방지 피처셋 요약:")
            LOGGER.info("=" * 60)
            
            removed_info = getattr(self.feature_selector, 'removed_features_info', {})
            
            LOGGER.info(f"📊 제거 요약 (Train 기준):")
            total_removed = 0
            ...
            LOGGER.info(f"\n🔧 파생 지표 피처 ({len(derived_features)}개):")
            for i, feature in enumerate(derived_features, 1):
                LOGGER.info(f"   {i:2d}. {feature}")
            
            LOGGER.info("=" * 60)
            LOGGER.info(f"🎯 Data Leakage 완전 방지: Train만으로 모든 통계 계산")
            LOGGER.info("=" * 60)
